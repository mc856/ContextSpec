...
Early prototype. v0.1 shipped to npm on 2026-05-25 (`contextspec@0.1.0`) as the first public test of the protocol's loop (curate → compile pack → run agent → retro → update memory). No telemetry; founder usage is currently the only signal. Whether the loop is worth shipping past v0.1 is still the open question.
