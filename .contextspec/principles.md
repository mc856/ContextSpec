...
- **Files are the source of truth.** Everything in `.contextspec/` is hand-written Markdown or YAML the user owns and diffs in git. The CLI never *edits* user-authored files (memory, roles, context) after they exist — scaffolding commands (`init`, `create-initiative`, `create-role`) create them and refuse to overwrite without `--force`. Beyond scaffolds it only writes generated artifacts (packs, slash commands, the managed AGENTS.md block) — and those carry explicit markers or live under `packs/`.
