...
## 2026-05-25 — v0.1.0 published to npm manually; tag-based auto-publish deferred to the next version

- decision: publish `contextspec@0.1.0` to the public npm registry manually, closing Phase 3. Verified `npm i -g contextspec` installs a working binary from the live registry. The published tarball is the slimmed 32-file payload (source maps dropped in `tsconfig.json`).
- alternative: publish via the `release.yml` tag flow instead. Rejected for the first release — the plan called for a manual first publish, and `release.yml` still needs its `NODE_AUTH_TOKEN` secret configured. Also, now that 0.1.0 exists on the registry, pushing a `v0.1.0` tag would re-trigger `release.yml` and fail on "cannot publish over an existing version"; tag-based auto-publish therefore begins at 0.1.1+.
- impact: Phase 3 acceptance is met. Phase 4's only remaining open item is the live synthetic-failure PR. Before the next tagged release, set the `NODE_AUTH_TOKEN` GitHub secret so `release.yml` can publish unattended.

## 2026-05-25 — synthetic-failure PR verified CI red; merge-gating left to convention at solo stage

- decision: close Phase 4 on observing the Test workflow go red (Node 20 + 22, `pull_request`) on a real PR carrying a failing test. Do **not** enable branch protection / required status checks on `main` for now.
- alternative: add a branch-protection rule requiring the Test checks before merge — the literal "cannot be merged" guarantee. Deferred — as a single-committer repo that pushes directly to `main`, the mechanical gate adds workflow friction for little marginal safety: a red run is visible on the PR and simply not merged. Revisit when a second committer joins.
- impact: the Phase 4 "cannot be merged" bullet is satisfied by convention, not enforcement; `acceptance.md` records the caveat so the doc does not overstate the guarantee. v0.1 (finish-line) is fully closed.
