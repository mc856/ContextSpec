...
- decision: publish `contextspec@0.1.0` to the public npm registry manually, closing Phase 3. Auth used a bypass-2FA access token because the account's interactive 2FA is a passkey (WebAuthn), which the `npm publish --otp=` flow cannot satisfy. Verified `npm i -g contextspec` installs a working binary from the live registry. The published tarball is the slimmed 32-file payload (source maps dropped in `tsconfig.json`).
- alternative: publish via the `release.yml` tag flow instead. Rejected for the first release — the plan called for a manual first publish, and `release.yml` still needs an `NPM_TOKEN` secret. Also, now that 0.1.0 exists on the registry, pushing a `v0.1.0` tag would re-trigger `release.yml` and fail on "cannot publish over an existing version"; tag-based auto-publish therefore begins at 0.1.1+.
- impact: Phase 3 acceptance is met. Phase 4's only remaining open item is the live synthetic-failure PR. Before the next tagged release, set the `NPM_TOKEN` GitHub secret (a granular or automation token) so `release.yml` can publish unattended.
